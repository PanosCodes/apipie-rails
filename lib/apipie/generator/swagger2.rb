...
module Apipie
  module Generator
    class Swagger2
      ...
      # @param [Hash] config
      def initialize(apipie, config = Apipie::Generator::Swagger2Config.instance.to_hash)
        @apipie = apipie
        @config = config
        @issued_warnings = []
      end

      def generate(version, resource_name, method_name, lang)
        init_swagger_vars(version, lang)
        ...
        if @config[:generate_x_computed_id_field]
      ...
      def params_in_body?
        @config[:content_type_input] == :json
      end

      def params_in_body_use_reference?
        @config[:json_input_uses_refs]
      end

      def responses_use_reference?
        @config[:responses_use_refs]
      end

      def include_warning_tags?
        @config[:include_warning_tags]
      end
      ...
      def init_swagger_vars(version, lang)
        ...
        @swagger = {
          ...
          security: @config[:global_security]
        }

        if @config[:api_host]
          @swagger[:host] = @config[:api_host]
        end
      ...
      # @param [Hash{String->Apipie::ResourceDescription}] resources
      ...
      def warn_missing_method_summary()
        warn 100, "missing short description for method";
      end

      def warn_added_missing_slash(path)
        warn 101, "added missing / at beginning of path: #{path}";
      end

      def warn_no_return_codes_specified()
        warn 102, "no return codes ('errors') specified";
      end

      def warn_hash_without_internal_typespec(param_name)
        warn 103, "the parameter :#{param_name} is a generic Hash without an internal type specification";
      end

      def warn_optional_param_in_path(param_name)
        warn 104, "the parameter :#{param_name} is 'in-path'.  Ignoring 'not required' in DSL";
      end

      def warn_optional_without_default_value(param_name)
        warn 105, "the parameter :#{param_name} is optional but default value is not specified (use :default_value => ...)";
      end

      def warn_param_ignored_in_form_data(param_name)
        warn 106, "ignoring param :#{param_name} -- cannot include Hash without fields in a formData specification";
      end

      def warn_path_parameter_not_described(name, path)
        warn 107, "the parameter :#{name} appears in the path #{path} but is not described";
      end

      def warn_inferring_boolean(name)
        warn 108, "the parameter [#{name}] is Enum with [true,false] values. Inferring 'boolean'";
      end

      def warn(warning_num, msg)
        suppress = @config[:suppress_warnings]
      ...
      def include_op_id_in_computed_interface_id(op_id)
        @computed_interface_id = Zlib::crc32("#{@computed_interface_id} #{op_id}") if @config[:generate_x_computed_id_field]
      end
      ...
      def json_schema_obj_from_params_array(params_array, allow_nulls = false)
        ...
        result[:additionalProperties] = false unless @config[:allow_additional_properties_in_response]
